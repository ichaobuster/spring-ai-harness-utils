 ...
 * @author ichaobuster
 */
public class AutoMemoryTools implements FileEditTool {

	private final StorageProvider memoryStorage;

	protected AutoMemoryTools(StorageProvider memoryStorage) {
		Assert.notNull(memoryStorage, "memoryStorage must not be null");
		this.memoryStorage = memoryStorage;
	}

	public StorageProvider getMemoryStorage() {
	...
	public String memoryView(
		...
		try {
			...
			if (this.memoryStorage.isDirectory(path)) {
				List<StorageProvider.Info> directoryContents = this.memoryStorage.listDirectory(path);
				return "Contents of %s:\n\n%s".formatted(path, formatListDirectoryContents(directoryContents));
			} else {
	...
	private String formatListDirectoryContents(List<StorageProvider.Info> directoryContents) {
		StringBuilder sb = new StringBuilder();
		for (StorageProvider.Info contentInfo : directoryContents) {
			String name = contentInfo.path();
			if (contentInfo.isDirectory()) {
				sb.append("  ").append(name).append(name.endsWith("/") || name.endsWith("\\") ? "" : "/\n");
			} else {
				sb.append("  ").append(name).append(" (").append(contentInfo.size()).append(" bytes)\n");
			}
		}
		return sb.toString();
	}
