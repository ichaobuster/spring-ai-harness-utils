...
class HumanInTheLoopBeforeAdvisorTest {
	...
	class BuilderTests {
		...
		void customOrder() {
			HumanInTheLoopBeforeAdvisor a = HumanInTheLoopBeforeAdvisor.builder(chatMemory).needPermissionTools(Set.of("foo")).order(42).build();
		...
		void toolCallingManager() {
			HumanInTheLoopBeforeAdvisor a = HumanInTheLoopBeforeAdvisor.builder(chatMemory)
	...
	void afterPassesThrough() {
		...
		HumanInTheLoopBeforeAdvisor a = HumanInTheLoopBeforeAdvisor.builder(chatMemory).needPermissionTools(Set.of("foo")).build();
	...
	class InterceptionTests {
		...
		void hitlDenied() {
			...
			ChatClientRequest request = request(new Prompt(new ArrayList<>(List.of(
					...
					opts), Map.of(HumanInTheLoopBeforeAdvisor.HITL_RESPONSE_KEY, hitlResponse));
	...
	void hitlAlwaysAllowed() {
		...
		ChatClientRequest request = request(new Prompt(new ArrayList<>(List.of(
				...
				opts), Map.of(HumanInTheLoopBeforeAdvisor.HITL_RESPONSE_KEY, hitlResponse,
				HumanInTheLoopBeforeAdvisor.HITL_ALWAYS_ALLOW_TOOLS_KEY, Set.of("foo")
		));
