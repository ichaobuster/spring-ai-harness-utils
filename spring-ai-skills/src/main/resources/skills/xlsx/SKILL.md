...
tool-calls: downloadOssFileToLocal uploadLocalFileToOss readXlsxPreview readXlsxSheet createXlsx editXlsxCells evaluateXlsxFormulas convertCsvToXlsx
license: Proprietary. LICENSE.txt has complete terms
---

# XLSX creation, editing, and analysis

All XLSX tool paths are **local filesystem paths** (absolute paths under `/tmp` are typical).
If the source file lives in Aliyun OSS, call `downloadOssFileToLocal` first, then pass the returned local path into these tools. After edits, call `uploadLocalFileToOss` to push results back.

| Task | Tool / Approach |
|---|---|
| **Materialize OSS → local** | `downloadOssFileToLocal` — download `prefix + path` into `{downloadPath}/{uuid}/{fileName}` and return absolute path |
| **Persist local → OSS** | `uploadLocalFileToOss` — upload a local result file back to `prefix + path` |
